...
# linguistic variables = [(TT, TI, TF), (IT, II, IF), (FT, FI, FF)]
# The type 2 neutrosophic number scale for relative importance of each criteria
wi = [
    [0.2, 0.3, 0.2],
    [0.6, 0.7, 0.8],
    [0.45, 0.75, 0.75]
...
# Linguistic variables for the classification
...
# equation 3 - mutliplying a scalar with a T2NN
...
def get_aggregated_opinion_matrix(opinion_matrix):
    ...
    for j in range(criteria_number):
        column = multiplied_matrix[:, j] # Loop over the columns
...
def calculate_npis(weighted_alternatives, benefit_indices, cost_indices):
    ...
    # For benefit criteria, take max across alternatives
    for p in benefit_indices:
        npis[p] = np.max(weighted_alternatives[:, p])
        
    # For cost criteria, take min across alternatives
...
def calculate_nnis(weighted_alternatives, benefit_indices, cost_indices):
    ...
    # For benefit criteria, take min across alternatives
    for p in benefit_indices:
        nnis[p] = np.min(weighted_alternatives[:, p])
        
    # For cost criteria, take max across alternatives
    for p in cost_indices:
